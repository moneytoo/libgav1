...
void SearchStack(const Tile::Block& block, const BlockParameters& mv_bp,
  ...
  if (is_compound) {
    ...
    if (IsGlobalMvBlock(candidate_mode, global_motion_type, candidate_size)) {
      ...
      candidate_mv.mv[0] = mv_bp.mv[index];
    }
    candidate_mv.mv[1] = {};
  }
  ...
  if (is_compound) {
    ...
    if (*num_mv_found == 1) {
      if (combined_mvs[0].mv64 == ref_mv_stack[0].mv64) {
        ref_mv_stack[1] = {{{combined_mvs[1].mv[0], combined_mvs[1].mv[1]}}, 2};
      } else {
        ref_mv_stack[1] = {{{combined_mvs[0].mv[0], combined_mvs[0].mv[1]}}, 2};
      }
      ++*num_mv_found;
    } else {
      ...
      *num_mv_found = 2;
      for (int i = 0; i < 2; ++i) {
        ref_mv_stack[i] = {{{combined_mvs[i].mv[0], combined_mvs[i].mv[1]}}, 2};
      }
    }
  } else {
