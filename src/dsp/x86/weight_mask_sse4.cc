...
template <bool mask_is_inverse>
inline void WeightMask8_10bpp_SSE4(const uint16_t* prediction_0,
                                   const uint16_t* prediction_1,
                                   uint8_t* mask) {
  const __m128i pred_0 = LoadAligned16(prediction_0);
  const __m128i pred_1 = LoadAligned16(prediction_1);
  const __m128i diff = RightShiftWithRounding_U16(
      _mm_abs_epi16(_mm_sub_epi16(pred_0, pred_1)), kRoundingBits10bpp);
  const __m128i scaled_diff = _mm_srli_epi16(diff, kScaledDiffShift);
  const __m128i adjusted_diff =
      _mm_adds_epu8(_mm_packus_epi16(scaled_diff, scaled_diff), diff_offset);
...
inline void WeightMask16_10bpp_SSE4(const uint16_t* prediction_0,
                                    const uint16_t* prediction_1,
                                    uint8_t* mask) {
  const __m128i pred_lo_0 = LoadAligned16(prediction_0);
  const __m128i pred_lo_1 = LoadAligned16(prediction_1);
  const __m128i diff0 = RightShiftWithRounding_U16(
      _mm_abs_epi16(_mm_sub_epi16(pred_lo_0, pred_lo_1)), kRoundingBits10bpp);
  const __m128i scaled_diff0 = _mm_srli_epi16(diff0, kScaledDiffShift);

  const __m128i pred_hi_0 = LoadAligned16(prediction_0 + 8);
  const __m128i pred_hi_1 = LoadAligned16(prediction_1 + 8);
  const __m128i diff1 = RightShiftWithRounding_U16(
      _mm_abs_epi16(_mm_sub_epi16(pred_hi_0, pred_hi_1)), kRoundingBits10bpp);
  const __m128i scaled_diff1 = _mm_srli_epi16(diff1, kScaledDiffShift);

  const __m128i scaled_diff = _mm_packus_epi16(scaled_diff0, scaled_diff1);
  const __m128i adjusted_diff = _mm_adds_epu8(scaled_diff, diff_offset);
  const __m128i mask_value = _mm_min_epi8(adjusted_diff, mask_ceiling);
  if (mask_is_inverse) {
    const __m128i inverted_mask_value = _mm_sub_epi8(mask_ceiling, mask_value);
...
#define WEIGHT8_WITHOUT_STRIDE_10BPP \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0, pred_1, mask)

#define WEIGHT8_AND_STRIDE_10BPP \
  WEIGHT8_WITHOUT_STRIDE_10BPP;  \
  pred_0 += 8;                   \
  pred_1 += 8;                   \
  mask += mask_stride
...
#define WEIGHT16_WITHOUT_STRIDE_10BPP \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0, pred_1, mask)
...
#define WEIGHT32_WITHOUT_STRIDE_10BPP                             \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0, pred_1, mask); \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0 + 16, pred_1 + 16, mask + 16)
...
#define WEIGHT64_WITHOUT_STRIDE_10BPP                                \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0, pred_1, mask);    \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0 + 16, pred_1 + 16, \
                                           mask + 16);               \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0 + 32, pred_1 + 32, \
                                           mask + 32);               \
  WeightMask16_10bpp_SSE4<mask_is_inverse>(pred_0 + 48, pred_1 + 48, mask + 48)
