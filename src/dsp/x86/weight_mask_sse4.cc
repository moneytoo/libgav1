...
inline void WeightMask16_10bpp_SSE4(const uint16_t* prediction_0,
  ...
  // precision of prediction: [3988, 61352]
  const __m128i pred_00 = LoadAligned16(prediction_0);
  const __m128i pred_10 = LoadAligned16(prediction_1);
  const __m128i pred_lo_00 = _mm_unpacklo_epi16(pred_00, zero);
  const __m128i pred_lo_10 = _mm_unpacklo_epi16(pred_10, zero);
  ...
  const __m128i pred_lo_01 = _mm_unpacklo_epi16(pred_01, zero);
  const __m128i pred_lo_11 = _mm_unpacklo_epi16(pred_11, zero);
