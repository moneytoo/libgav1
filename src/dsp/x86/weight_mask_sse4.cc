...
inline void WeightMask8_10bpp_SSE4(const uint16_t* prediction_0,
  ...
  const __m128i scaled_difference =
      _mm_srli_epi16(difference, kScaledDiffShift);
  const __m128i adjusted_difference =
      _mm_adds_epu8(_mm_packus_epi16(scaled_difference, scaled_difference),
                    difference_offset);
  const __m128i mask_value = _mm_min_epi8(adjusted_difference, mask_ceiling);
...
#define WEIGHT16_WITHOUT_STRIDE_10BPP                            \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0, pred_1, mask); \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 8, pred_1 + 8, mask + 8)
...
#define WEIGHT32_WITHOUT_STRIDE_10BPP                                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0, pred_1, mask);             \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 8, pred_1 + 8, mask + 8); \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 16, pred_1 + 16,          \
                                          mask + 16);                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 24, pred_1 + 24, mask + 24)
...
#define WEIGHT64_WITHOUT_STRIDE_10BPP                                        \
  ...
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 16, pred_1 + 16,          \
                                          mask + 16);                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 24, pred_1 + 24,          \
                                          mask + 24);                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 32, pred_1 + 32,          \
                                          mask + 32);                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 40, pred_1 + 40,          \
                                          mask + 40);                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 48, pred_1 + 48,          \
                                          mask + 48);                        \
  WeightMask8_10bpp_SSE4<mask_is_inverse>(pred_0 + 56, pred_1 + 56, mask + 56)
