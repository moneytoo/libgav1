...
inline __m128i GetMask4x2_S(const uint8_t* mask, ptrdiff_t mask_stride,
                            const __m128i zero) {
  (void)zero;
  const __m128i mask_val_0 = Load4(mask);
  const __m128i mask_val_1 = Load4(mask + mask_stride);
...
inline __m128i GetMask4x2_Sx(const uint8_t* mask, ptrdiff_t mask_stride,
                             const __m128i zero) {
  const __m128i mask_val_0 = _mm_cvtepu8_epi16(LoadLo8(mask));
...
inline __m128i GetMask4x2_Sx_Sy(const uint8_t* mask, ptrdiff_t mask_stride,
                                const __m128i zero) {
...
inline __m128i GetMask8_S(const uint8_t* mask, ptrdiff_t stride,
                          const __m128i zero) {
  (void)zero;
  (void)stride;
  const __m128i mask_val = LoadLo8(mask);
...
inline __m128i GetMask8_Sx(const uint8_t* mask, ptrdiff_t stride,
                           const __m128i zero) {
  (void)stride;
  const __m128i row_vals = LoadUnaligned16(mask);
  const __m128i mask_val_0 = _mm_cvtepu8_epi16(row_vals);
...
inline __m128i GetMask8_Sx_Sy(const uint8_t* mask, ptrdiff_t mask_stride,
                              const __m128i zero) {
  const __m128i one = _mm_set1_epi8(1);
...
typedef __m128i (*GetMasks)(const uint8_t* mask, ptrdiff_t stride,
                            const __m128i zero);
static const GetMasks GetMask[2][3] = {
...
inline void InterIntraWriteMaskBlendLine10bpp4x2_SSE4_1(
    const uint16_t* const pred_0, const uint16_t* const pred_1,
    const ptrdiff_t pred_stride_1, const __m128i pred_mask_0,
    const __m128i pred_mask_1, uint16_t* dst, const ptrdiff_t dst_stride,
    const __m128i shift) {
...
inline void WriteMaskBlendLine10bpp4x2_SSE4_1(
    ...
    const ptrdiff_t pred_stride_1, const __m128i pred_mask_0,
    const __m128i pred_mask_1, uint16_t* dst, const ptrdiff_t dst_stride,
    const __m128i offset, const __m128i zero, const __m128i max,
    const __m128i shift4) {
  const __m128i pred_val_0 = LoadUnaligned16(pred_0);
  const __m128i pred_val_1 =
      _mm_set_epi64x(*(uint64_t*)(pred_1 + pred_stride_1), *(uint64_t*)pred_1);
...
inline void MaskBlend10bpp_SSE4_1(const void* prediction_0,
  ...
  int y = 0;
  const __m128i offset = _mm_set1_epi32(kCompoundOffset);
  const __m128i offset_16b = _mm_set1_epi16(kCompoundOffset);
  const __m128i max = _mm_set1_epi16(1023);
  const __m128i shift4 = _mm_set1_epi32((1 << 4) >> 1);
  do {
    int x = 0;
