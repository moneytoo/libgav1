  ...
  if (subsampling_x == 1) {
    if (subsampling_y == 0) {
      ...
      const __m128i mask_val_0 = _mm_set_epi64x(
          *reinterpret_cast<const uint64_t*>(mask + (mask_stride << 1)),
          *reinterpret_cast<const uint64_t*>(mask));
      const __m128i mask_val_1 = _mm_set_epi64x(
          *reinterpret_cast<const uint64_t*>(mask + (mask_stride << 1) +
                                             mask_stride),
          *reinterpret_cast<const uint64_t*>(mask + mask_stride));
...
inline void WriteMaskBlendLine10bpp4x2_SSE4_1(
  ...
  const __m128i pred_val_1 =
      _mm_set_epi64x(*reinterpret_cast<const uint64_t*>(pred_1 + pred_stride_1),
                     *reinterpret_cast<const uint64_t*>(pred_1));
...
inline void InterIntraWriteMaskBlendLine10bpp4x2_SSE4_1(
  ...
  const __m128i pred_val_1 = _mm_set_epi64x(
      *reinterpret_cast<const uint64_t*>(prediction_1 + pred_stride_1),
      *reinterpret_cast<const uint64_t*>(prediction_1));
...
#endif
#if DSP_ENABLED_10BPP_SSE4_1(InterIntraMaskBlend444)
  dsp->mask_blend[0][1] = InterIntraMaskBlend10bpp_SSE4_1<0, 0>;
#endif
#if DSP_ENABLED_10BPP_SSE4_1(InterIntraMaskBlend422)
  dsp->mask_blend[1][1] = InterIntraMaskBlend10bpp_SSE4_1<1, 0>;
#endif
#if DSP_ENABLED_10BPP_SSE4_1(InterIntraMaskBlend420)
