...
using GetMasks = __m128i (*)(const uint8_t* mask, const ptrdiff_t stride,
                             const __m128i zero);
...
inline __m128i GetMask4x2_S(const uint8_t* mask, const ptrdiff_t mask_stride,
                            const __m128i /*zero*/) {
  const __m128i mask_val_0 = Load4(mask);
  const __m128i mask_val_1 = Load4(mask + mask_stride);
  return _mm_cvtepu8_epi16(
...
inline __m128i GetMask4x2_Sx(const uint8_t* mask, const ptrdiff_t mask_stride,
                             const __m128i zero) {
  const __m128i mask_val_0 = _mm_cvtepu8_epi16(LoadLo8(mask));
...
inline __m128i GetMask4x2_Sx_Sy(const uint8_t* mask,
                                const ptrdiff_t mask_stride,
                                const __m128i zero) {
...
inline __m128i GetMask8_S(const uint8_t* mask, const ptrdiff_t /*stride*/,
                          const __m128i /*zero*/) {
  const __m128i mask_val = LoadLo8(mask);
  return _mm_cvtepu8_epi16(mask_val);
}

inline __m128i GetMask8_Sx(const uint8_t* mask, const ptrdiff_t /*stride*/,
                           const __m128i zero) {
  const __m128i row_vals = LoadUnaligned16(mask);
  const __m128i mask_val_0 = _mm_cvtepu8_epi16(row_vals);
  const __m128i mask_val_1 = _mm_cvtepu8_epi16(_mm_srli_si128(row_vals, 8));
...
inline __m128i GetMask8_Sx_Sy(const uint8_t* mask, const ptrdiff_t mask_stride,
                              const __m128i zero) {
  const __m128i one = _mm_set1_epi8(1);
...
GetMasks GetMask[2][3] = {{GetMask4x2_S, GetMask4x2_Sx, GetMask4x2_Sx_Sy},
...
inline void InterIntraWriteMaskBlendLine10bpp4x2_SSE4_1(
    ...
    uint16_t* dst, const ptrdiff_t dst_stride, const __m128i shift6) {
  ...
  const __m128i compound_pred_lo_0 = _mm_mullo_epi16(pred_val_1, pred_mask_0);
  const __m128i compound_pred_hi_0 = _mm_mulhi_epu16(pred_val_1, pred_mask_0);
  const __m128i compound_pred_lo_1 = _mm_mullo_epi16(pred_val_0, pred_mask_1);
  const __m128i compound_pred_hi_1 = _mm_mulhi_epu16(pred_val_0, pred_mask_1);
  ...
  const __m128i shift_0 =
      RightShiftWithRoundingConst_S32(compound_pred_lo, 6, shift6);
  const __m128i shift_1 =
      RightShiftWithRoundingConst_S32(compound_pred_hi, 6, shift6);
...
inline void MaskBlending10bpp4x4_SSE4_1(
  ...
  __m128i pred_mask_0 =
      GetMask[0][subsampling_x + subsampling_y](mask, mask_stride, zero);
  __m128i pred_mask_1 = _mm_sub_epi16(mask_inverter, pred_mask_0);
...
template <int subsampling_x, int subsampling_y>
inline void MaskBlending10bpp4xH_SSE4_1(const uint16_t* pred_0,
...
inline void MaskBlend10bpp_SSE4_1(const void* prediction_0,
  ...
  do {
    int x = 0;
    do {
      const __m128i pred_mask_0 = GetMask[1][subsampling_x + subsampling_y](
...
inline void InterIntraMaskBlend10bpp_SSE4_1(
  ...
  do {
    ...
    do {
      ...
      const __m128i pred_mask_1 = _mm_sub_epi16(mask_inverter, pred_mask_0);
      const __m128i compound_pred_lo_0 =
          _mm_mullo_epi16(pred_val_1, pred_mask_0);
      const __m128i compound_pred_hi_0 =
          _mm_mulhi_epu16(pred_val_1, pred_mask_0);
      const __m128i compound_pred_lo_1 =
          _mm_mullo_epi16(pred_val_0, pred_mask_1);
      const __m128i compound_pred_hi_1 =
          _mm_mulhi_epu16(pred_val_0, pred_mask_1);
      const __m128i pack0_lo =
          _mm_unpacklo_epi16(compound_pred_lo_0, compound_pred_hi_0);
      const __m128i pack0_hi =
          _mm_unpackhi_epi16(compound_pred_lo_0, compound_pred_hi_0);
