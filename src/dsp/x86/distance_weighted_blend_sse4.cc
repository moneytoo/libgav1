...
// This offset if combination of round_factor and round_offset
// which are to be added and subtracted respectively.
// Here kInterPostRoundBit + 4 is 8 (considering bitdepth=10)
constexpr int kOffset = ((1 << (8 - 1)) - (kCompoundOffset << 4));
constexpr int kMax10BppSample = ((1 << 10) - 1);
constexpr int kInterPostRoundBit = 4;

inline __m128i ComputeWeightedAverage8_10bpp(const __m128i& pred0,
                                             const __m128i& pred1,
                                             const __m128i& weight0,
                                             const __m128i& weight1) {
  const __m128i zero = _mm_set1_epi32(kZero);
  const __m128i bias = _mm_set1_epi32(kOffset);
  const __m128i clip_low = _mm_set1_epi16(kZero);
  const __m128i clip_high = _mm_set1_epi16(kMax10BppSample);

  __m128i prediction0 = _mm_unpacklo_epi16(pred0, zero);
  __m128i mult0 = _mm_mullo_epi32(prediction0, weight0);
  __m128i prediction1 = _mm_unpacklo_epi16(pred1, zero);
  ...
  const __m128i pack = _mm_packs_epi32(result0, result1);

  return _mm_min_epi16(_mm_max_epi16(pack, clip_low), clip_high);
}

template <int height>
inline void DistanceWeightedBlend4xH_10bpp_SSE4_1(
  ...
  for (int y = 0; y < height; y += 4) {
    ...
    pred_1 += 4;
    const __m128i res0 =
        ComputeWeightedAverage8_10bpp(src_0, src_1, weight0, weight1);
...
template <int height>
inline void DistanceWeightedBlend8xH_10bpp_SSE4_1(
    const uint16_t* pred_0, const uint16_t* pred_1, const uint8_t weight_0,
    const uint8_t weight_1, void* const dest, const ptrdiff_t dest_stride) {
  auto* dst = static_cast<uint16_t*>(dest);
  const __m128i weight0 = _mm_set1_epi32(weight_0);
  ...
  for (int y = 0; y < height; y += 2) {
    ...
    pred_1 += 8;
    const __m128i res0 =
        ComputeWeightedAverage8_10bpp(src_00, src_10, weight0, weight1);
    ...
    pred_1 += 8;
    const __m128i res1 =
        ComputeWeightedAverage8_10bpp(src_01, src_11, weight0, weight1);
...
inline void DistanceWeightedBlendLarge_10bpp_SSE4_1(
    const uint16_t* pred_0, const uint16_t* pred_1, const uint8_t weight_0,
    const uint8_t weight_1, const int width, const int height, void* const dest,
    const ptrdiff_t dest_stride) {
  auto* dst = static_cast<uint16_t*>(dest);
  ...
  do {
    ...
    do {
      ...
      const __m128i src_1_lo = LoadAligned16(pred_1 + x);
      const __m128i res_lo =
          ComputeWeightedAverage8_10bpp(src_0_lo, src_1_lo, weight0, weight1);

      const __m128i src_0_hi = LoadAligned16(pred_0 + x + 8);
      const __m128i src_1_hi = LoadAligned16(pred_1 + x + 8);
      const __m128i res_hi =
          ComputeWeightedAverage8_10bpp(src_0_hi, src_1_hi, weight0, weight1);
...
void DistanceWeightedBlend_10bpp_SSE4_1(const void* prediction_0,
                                        const void* prediction_1,
                                        const uint8_t weight_0,
                                        const uint8_t weight_1, const int width,
                                        const int height, void* const dest,
  ...
  if (width == 4) {
    if (height == 4) {
      DistanceWeightedBlend4xH_10bpp_SSE4_1<4>(pred_0, pred_1, weight_0,
                                               weight_1, dest, dst_stride);
    } else if (height == 8) {
      DistanceWeightedBlend4xH_10bpp_SSE4_1<8>(pred_0, pred_1, weight_0,
                                               weight_1, dest, dst_stride);
    } else {
      assert(height == 16);
      DistanceWeightedBlend4xH_10bpp_SSE4_1<16>(pred_0, pred_1, weight_0,
                                                weight_1, dest, dst_stride);
    }
    return;
  }
  ...
  if (width == 8) {
    switch (height) {
      case 4:
        DistanceWeightedBlend8xH_10bpp_SSE4_1<4>(pred_0, pred_1, weight_0,
                                                 weight_1, dest, dst_stride);
        return;
      case 8:
        DistanceWeightedBlend8xH_10bpp_SSE4_1<8>(pred_0, pred_1, weight_0,
                                                 weight_1, dest, dst_stride);
        return;
      case 16:
        DistanceWeightedBlend8xH_10bpp_SSE4_1<16>(pred_0, pred_1, weight_0,
                                                  weight_1, dest, dst_stride);
        return;
      default:
        assert(height == 32);
        DistanceWeightedBlend8xH_10bpp_SSE4_1<32>(pred_0, pred_1, weight_0,
                                                  weight_1, dest, dst_stride);
  ...
  DistanceWeightedBlendLarge_10bpp_SSE4_1(pred_0, pred_1, weight_0, weight_1,
                                          width, height, dest, dst_stride);
}
...
void Init10bpp() {
  ...
  dsp->distance_weighted_blend = DistanceWeightedBlend_10bpp_SSE4_1;
#endif
}
