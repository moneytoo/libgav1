...
namespace high_bitdepth {
	namespace {
		...
		void CflSubsampler420_4xH_SSE4_1(
			...
			const void* const source, ptrdiff_t stride) {
			const int block_height = 1 << block_height_log2;
			const auto* src = static_cast<const int16_t*>(source);
			...
			do {
				// Note that double sampling and converting to 16bit makes a row fill the
				// vector.
				const __m128i samples_row0_check = LoadUnaligned16(src);
				const __m128i samples_row0 = (LoadUnaligned16(src)); //_mm_cvtepu8_epi16
				src += stride;
