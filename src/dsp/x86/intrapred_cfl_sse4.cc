...
void CflSubsampler420_4xH_SSE4_1(
  ...
  do {
    ...
    __m128i sum =
        utils::StoreLumaResults4_420(luma_sum01, luma_sum23, luma_ptr);
    ...
    sum = _mm_add_epi16(
        sum, utils::StoreLumaResults4_420(luma_sum45, luma_sum67, luma_ptr));
    luma_ptr += kCflLumaBufferStride << 1;
...
inline void CflSubsampler420Impl_8xH_SSE4_1(
  ...
  do {
    ...
    sum = _mm_add_epi16(
        sum, utils::StoreLumaResults8_420(luma_sum30, luma_sum31, luma_ptr));
    luma_ptr += kCflLumaBufferStride;
...
inline void CflSubsampler420Impl_WxH_SSE4_1(
  ...
  do {
    ...
    __m128i sum = utils::StoreLumaResults8_420(luma_sum0, luma_sum1, luma_ptr);
    final_row_result =
        utils::StoreLumaResults8_420(luma_sum2, luma_sum3, luma_ptr + 8);
    sum = _mm_add_epi16(sum, final_row_result);
    if (block_width_log2 == 5) {
      const __m128i wide_fill = utils::LastRowResult(final_row_result);
  ...
  if (y < block_height) {
    ...
    if (block_width_log2 == 5) {
      ...
      wide_fill = _mm_slli_epi32(
          _mm_cvtepi16_epi32(utils::LastRowResult(final_fill1)), 2);
    }
...
void CflSubsampler444_4xH_SSE4_1(
  ...
  int16_t* luma_ptr = luma[0];
  __m128i sum_even = _mm_setzero_si128();
  __m128i sum_odd = _mm_setzero_si128();
  __m128i sum;
  __m128i samples;
  ...
  do {
    samples = LoadLo8(src);
    src += src_stride;
    sum_even = _mm_add_epi16(sum_even, samples);

    samples = LoadLo8(src);
    src += src_stride;
    sum_odd = _mm_add_epi16(sum_odd, samples);
    y -= 2;
  } while (y != 0);
  ...
  sum =
      _mm_add_epi32(_mm_cvtepu16_epi32(sum_even), _mm_cvtepu16_epi32(sum_odd));
  sum = _mm_add_epi32(sum, _mm_srli_si128(sum, 8));
  sum = _mm_add_epi32(sum, _mm_srli_si128(sum, 4));
...
void CflSubsampler444_8xH_SSE4_1(
  ...
  // Since the maximum height is 32 and minimum height is 8,
  // if we split them by factor of 8, we can store them in
  // 16 bits without casting to 32 bits.
  do {
    samples = LoadUnaligned16(src);
    src += src_stride;
    sum_0 = _mm_add_epi16(sum_0, samples);

    samples = LoadUnaligned16(src);
    src += src_stride;
    sum_1 = _mm_add_epi16(sum_1, samples);

    samples = LoadUnaligned16(src);
    src += src_stride;
    sum_2 = _mm_add_epi16(sum_2, samples);

    samples = LoadUnaligned16(src);
    src += src_stride;
    sum_3 = _mm_add_epi16(sum_3, samples);
    y -= 4;
  } while (y != 0);
  ...
  sum =
      _mm_add_epi32(_mm_unpackhi_epi16(sum_0, zero), _mm_cvtepu16_epi32(sum_0));
  sum = _mm_add_epi32(sum, _mm_unpackhi_epi16(sum_1, zero));
  sum = _mm_add_epi32(sum, _mm_cvtepu16_epi32(sum_1));
  sum = _mm_add_epi32(sum, _mm_unpackhi_epi16(sum_2, zero));
  sum = _mm_add_epi32(sum, _mm_cvtepu16_epi32(sum_2));
  sum = _mm_add_epi32(sum, _mm_unpackhi_epi16(sum_3, zero));
  sum = _mm_add_epi32(sum, _mm_cvtepu16_epi32(sum_3));
  ...
  do {
    ...
    for (int x = 0; x < block_width; x += 8) {
      if (max_luma_width > x) {
        ...
        samples[idx] = utils::LastRowResult(samples_ext);
      }
...
inline void CflSubsampler420Impl_WxH_SSE4_1(
  ...
  do {
    ...
    if (block_width_log2 == 5) {
      const __m128i wide_fill = utils::LastRowResult(final_row_result);
      // Multiply duplicated value by number of occurrences, * 4, since
  ...
  if (block_width_log2 == 5) {
    ...
    // there are 16 in each row and the value appears in the vector 4 times.
    final_sum =
        _mm_add_epi32(final_sum, _mm_slli_epi32(_mm_cvtepi16_epi32(wide_fill),
                                                (block_height_log2 - 1) + 2));
    switch ((block_height - max_luma_height) >> 1) {
      case 4:
        final_sum = _mm_add_epi32(
            final_sum, _mm_slli_epi32(_mm_cvtepi16_epi32(wide_fill), (2) + 2));
        break;
      case 8:
        final_sum = _mm_add_epi32(
            final_sum, _mm_slli_epi32(_mm_cvtepi16_epi32(wide_fill), (3) + 2));
        break;
      case 12:
        final_sum = _mm_add_epi32(
            final_sum, _mm_slli_epi32(_mm_cvtepi16_epi32(wide_fill), (4) + 2));
        final_sum = _mm_sub_epi32(
            final_sum, _mm_slli_epi32(_mm_cvtepi16_epi32(wide_fill), (2) + 2));
        break;
      default:
        assert((block_height - max_luma_height) == 0);
        break;
    }
  }
