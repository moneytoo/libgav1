...
void CflIntraPredictor_10bpp_SSE4_1(
  ...
  do {
    ...
    if (width >= 16) {
      const __m128i res_1 =
          CflPredictUnclipped(row + 1, alpha_q12, alpha_sign, dc_val);
      _mm_storeu_si128(((__m128i*)dst) + 1, clip_epi16(res_1, zeros, max));
    }
    if (width == 32) {
      const __m128i res_2 =
          CflPredictUnclipped(row + 2, alpha_q12, alpha_sign, dc_val);
      _mm_storeu_si128((__m128i*)(dst + 16), clip_epi16(res_2, zeros, max));
      const __m128i res_3 =
          CflPredictUnclipped(row + 3, alpha_q12, alpha_sign, dc_val);
      _mm_storeu_si128((__m128i*)(dst + 24), clip_epi16(res_3, zeros, max));
    }
    dst += stride;
  } while ((row += (1 << kCflLumaBufferStrideLog2_128i)) < row_end);
}
