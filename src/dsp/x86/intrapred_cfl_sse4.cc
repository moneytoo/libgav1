...
void CflSubsampler444_4xH_SSE4_1(
  ...
  do {
    __m128i samples = LoadLo8(src);
    src += src_stride;
    samples = _mm_slli_epi16(samples, 3);
    StoreUnaligned16(luma_ptr, samples);
    luma_ptr += kCflLumaBufferStride;
...
template <int block_height_log2>
void CflSubsampler444_4xH(
    int16_t luma[kCflLumaBufferStride][kCflLumaBufferStride],
    const int max_luma_width, const int max_luma_height,
    const void* const source, ptrdiff_t stride) {
  (void)max_luma_width;
  (void)max_luma_height;
  static_assert(block_height_log2 <= 4, "");
  assert(max_luma_width >= 4);
  assert(max_luma_height >= 4);

  CflSubsampler444_4xH_SSE4_1<block_height_log2>(luma, source, stride);
}
...
void CflSubsampler444_8xH_SSE4_1(
  ...
  do {
    __m128i samples = LoadUnaligned16(src);
    src += src_stride;
    samples = _mm_slli_epi16(samples, 3);
    StoreUnaligned16(luma_ptr, samples);
    luma_ptr += kCflLumaBufferStride;
...
void CflSubsampler444_8xH(
  ...
  CflSubsampler444_8xH_SSE4_1<block_height_log2>(luma, source, stride);
}
...
void CflSubsampler444_WxH_SSE4_1(
  ...
  do {
    __m128i samples0 = LoadUnaligned16(src);
    __m128i samples1 = LoadUnaligned16(src + 8);

    samples0 = _mm_slli_epi16(samples0, 3);
    samples1 = _mm_slli_epi16(samples1, 3);
    ...
    if (block_width == 32) {
      __m128i samples2 = LoadUnaligned16(src + 16);
      __m128i samples3 = LoadUnaligned16(src + 24);

      samples2 = _mm_slli_epi16(samples2, 3);
      samples3 = _mm_slli_epi16(samples3, 3);
...
void CflSubsampler444_WxH(
  ...
  CflSubsampler444_WxH_SSE4_1<block_width_log2, block_height_log2>(luma, source,
                                                                   stride);
}
