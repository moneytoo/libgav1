...
LIBGAV1_ALWAYS_INLINE void AddPartial_D1_D3(uint8x8_t* v_src,
                                            uint16x8_t* partial_lo,
                                            uint16x8_t* partial_hi) {
  uint16x8_t v_d1_temp[8];
  const uint8x8_t v_zero = vdup_n_u8(0);
  const uint16x8_t v_zero_16 = vdupq_n_u16(0);
  ...
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[1], 7));

  // 00 00 C0 C1 C2 C3 00 00
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[2], 6));
  // 00 00 00 D0 D1 D2 D3 00
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[3], 5));
  // 00 00 00 00 E0 E1 E2 E3
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[4], 4));

  // 00 00 00 00 00 F0 F1 F2
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[5], 3));
  // F3 00 00 00 00 00 00 00
  *partial_hi = vpadalq_u8(*partial_hi, vextq_u16(v_d1_temp[5], v_zero_16, 3));

  // 00 00 00 00 00 00 G0 G1
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[6], 2));
  // G2 G3 00 00 00 00 00 00
  *partial_hi = vpadalq_u8(*partial_hi, vextq_u16(v_d1_temp[6], v_zero_16, 2));

  // 00 00 00 00 00 00 00 H0
  *partial_lo = vpadalq_u8(*partial_lo, vextq_u16(v_zero_16, v_d1_temp[7], 1));
  // H1 H2 H3 00 00 00 00 00
  *partial_hi = vpadalq_u8(*partial_hi, vextq_u16(v_d1_temp[7], v_zero_16, 1));
}
